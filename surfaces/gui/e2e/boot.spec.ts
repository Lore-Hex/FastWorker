// Cold-boot fixes (owner-hit 2026-07-23): the splash wears the real TrustedRouter mark,
// not the old text glyph that read as another product's logo, and the
...
test("boot splash shows the TrustedRouter logo, not the sparkle glyph", async ({ page }) => {
  ...
  await expect(mark.locator('img[src*="fastworker-logo"]')).toBeVisible();
  await expect(mark).not.toContainText("✦");
  await expect(page.getByText(/Starting FastWorker|Restoring your session/)).toBeVisible();
});
